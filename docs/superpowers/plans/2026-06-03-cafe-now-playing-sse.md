...
for f in app/services/spotify.py app/api/cafe.py app/main.py \
          app/static/cafe_tablet.html app/static/ops_cafe.html; do
  scp $f __PROD_HOST__:~/apps/hahahoho-prod/$f
done
ssh __PROD_HOST__ "cd ~/apps/hahahoho-prod && python3 -m py_compile app/services/spotify.py app/api/cafe.py app/main.py && echo OK"
ssh __PROD_HOST__ "PID=\$(pgrep -f 'uvicorn.*8000'); kill -TERM \$PID; sleep 6; curl -s -w 'HTTP:%{http_code}' http://localhost:8000/"
```
예상: `HTTP:401`
