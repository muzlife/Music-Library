...
ssh matia@macmini2018.local "sqlite3 /Users/matia/apps/hahahoho-prod/runtime/data/library.db \
...
ssh matia@macmini2018.local \
  "sed -i.bak 's|^LIBRARY_AUTH_SESSION_SECRET=.*|LIBRARY_AUTH_SESSION_SECRET=\"$NEW_SECRET\"|' /Users/matia/apps/hahahoho-prod/.env"
```
...
### Production
```bash
ssh matia@macmini2018.local 'launchctl kickstart -k gui/$(id -u)/com.muzlife.library-prod && sleep 2 && curl -s http://127.0.0.1:8000/health'
# {"status":"ok"} 면 OK
```
