...
def _ensure_auth_account_table(conn: sqlite3.Connection) -> None:
    ...
    if row and row[0]:
        sql_schema = row[0]
        if "VIEWER" not in sql_schema or "CAFE_STAFF" not in sql_schema:
...
def list_auth_accounts() -> list[dict[str, Any]]:
    with get_conn() as conn:
        ...
        rows = conn.execute(
            """
            SELECT id, username, password_hash, role, is_active, display_name, description, created_at, updated_at
            FROM auth_account
            ORDER BY
...
def upsert_auth_account(
    ...
    if not key or not hashed or role_code not in {"ADMIN", "OPERATOR", "CAFE_STAFF", "VIEWER"}:
        return None
    now = utc_now_iso()
    with get_conn() as conn:
        _ensure_auth_account_table(conn)
        conn.execute(
            """
            INSERT INTO auth_account (username, password_hash, role, is_active, display_name, description, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ...
            """,
            (key, hashed, role_code, 1 if is_active else 0, display_name, description, now, now),
        )
