...
from ..services import purchase_mail as _pm
...
def preview_purchase_import(
    ...
    raw_content, _html_content = _pm._resolve_purchase_import_raw_input(payload)
    resolved_vendor_code = _pm._resolve_purchase_import_vendor_code(
        payload.vendor_code, raw_content=raw_content
    )
    items = _pm._parse_purchase_import_preview(payload)
    if not items:
        reason = _pm._purchase_import_empty_reason(resolved_vendor_code, raw_content)
        if reason:
...
def save_purchase_import(
    ...
    resolved_purchase_date = _pm._resolve_purchase_import_purchase_date(
        payload.purchase_date, items=payload.items
    )
    rows = _pm._purchase_import_rows_for_save(
        payload.items,
        vendor_code=resolved_vendor_code,
    ...
    created_ids = db.insert_purchase_import_rows(
        ...
        payload.source_type,
        rows,
        source_ref=_pm._clean_text(payload.source_ref),
        email_from=_pm._clean_text(payload.email_from),
        email_subject=_pm._clean_text(payload.email_subject),
...
def _webhook_envelope_dependency(request: Request) -> None:
    ...
    _pm._require_purchase_import_webhook_envelope(request)
...
def purchase_import_webhook_gmail(
    ...
    cleaned_source_ref = _pm._clean_text(payload.source_ref)
...
def list_purchase_imports(
    ...
    return PurchaseImportListResponse(
        ...
        items=[_pm._purchase_queue_item_from_row(row) for row in rows],
    )
...
def list_purchase_import_candidates(
    ...
    item_url = _pm._clean_text(row.get("item_url"))
    has_artist_name = bool(_pm._clean_text(row.get("artist_name")))
    has_image_url = bool(_pm._clean_text(row.get("image_url")))
    if vendor_code == "AMAZON" and item_url and (not has_artist_name or not has_image_url):
        try:
            working_row = _pm._purchase_enrich_row_from_item_page(row)
        except Exception as exc:
            logger.warning("amazon page enrich failed for queue_id=%d: %s", queue_id, exc)
            working_row = row

    search_query = _pm._purchase_queue_candidate_query(
    ...
    candidates = main_module._search_lookup_metadata_candidates(
        ...
        limit=limit,
        artist_or_brand=_pm._clean_text(artist_name)
        if artist_name is not None
        else _pm._clean_text(working_row.get("artist_name")),
        title=_pm._clean_text(item_name)
...
def enrich_purchase_import_from_item_page(
    ...
    if row is None:
        raise HTTPException(status_code=404, detail="purchase import row not found")
    updated = _pm._purchase_enrich_row_from_item_page(row)
    return _pm._purchase_queue_item_from_row(updated)
...
def create_owned_item_from_purchase_import(
    ...
    if duplicate_row is not None:
        ...
        if existing_owned_item_id > 0 and db.get_owned_item(existing_owned_item_id) is not None:
            return _pm._purchase_import_duplicate_create_response(
                queue_id=queue_id,
                row=row,
                existing_owned_item_id=existing_owned_item_id,
            )
    payload = _pm._build_owned_item_from_purchase_queue_row(row)
    ...
    return PurchaseImportCreateResponse(
        queue_item=_pm._purchase_queue_item_from_row(updated),
...
def create_owned_item_from_purchase_import_candidate(
    ...
    try:
        owned_payload = _pm._build_owned_item_from_purchase_queue_row(row, candidate)
