...
_CLIMATE_REFRESH_STOP_EVENT = threading.Event()
_CLIMATE_REFRESH_THREAD: threading.Thread | None = None
...
def _fetch_office_climate_to_cache() -> None:
...
def _load_operator_office_climate() -> dict[str, Any]:
    now = time.monotonic()
    if _OFFICE_CLIMATE_CACHE_DATA is not None and (now - _OFFICE_CLIMATE_CACHE_TS) < _OFFICE_CLIMATE_TTL:
...
def _fetch_seoul_weather_to_cache() -> None:
...
def _load_operator_seoul_weather() -> dict[str, Any]:
    now = time.monotonic()
    if _SEOUL_WEATHER_CACHE_DATA is not None and (now - _SEOUL_WEATHER_CACHE_TS) < _SEOUL_WEATHER_TTL:
...
def _climate_refresh_worker() -> None:
    while not _CLIMATE_REFRESH_STOP_EVENT.wait(55):
        try:
            _fetch_office_climate_to_cache()
        except Exception:
            pass
        now = time.monotonic()
        if now - _SEOUL_WEATHER_CACHE_TS >= _SEOUL_WEATHER_TTL - 30:
            try:
                _fetch_seoul_weather_to_cache()
            except Exception:
                pass


def _start_climate_refresh_worker() -> None:
    global _CLIMATE_REFRESH_THREAD
    if _CLIMATE_REFRESH_THREAD is not None and _CLIMATE_REFRESH_THREAD.is_alive():
        return
    _CLIMATE_REFRESH_STOP_EVENT.clear()
    _CLIMATE_REFRESH_THREAD = threading.Thread(
        target=_climate_refresh_worker,
        name="climate-refresh-worker",
        daemon=True,
    )
    _CLIMATE_REFRESH_THREAD.start()
